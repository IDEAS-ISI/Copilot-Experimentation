...
class SchemaValidator:
    """
    Validates generated SQL queries against the predefined database schema.
    It checks for valid tables, columns, and date operations to prevent execution errors.
    """
    def __init__(self):
        self.schema = {
    ...
    def extract_table_aliases(self, sql):
        """
        Extracts table aliases from a given SQL query string.
        Looks for 'FROM table AS alias' or 'JOIN table [AS] alias' patterns.
        
        Args:
            sql (str): The SQL query string.
            
        Returns:
            dict: A mapping of aliases to their corresponding table names.
        """
    ...
    def validate(self, sql):
        """
        Validates the SQL query by checking column existence and allowed date operations.
...
def get_db_credentials():
    """
    Retrieves database credentials from environment variables or prompts the user.
    Prioritizes .env settings over manual user input.
    
    Returns:
        dict: A dictionary containing the database connection configuration.
    """
...
def interactive_mode():
    """
    Main loop for interactive inference mode.
    Handles user input, intent classification, dynamic SQL generation, and query execution.
    It routing traffic between a General LLM and a specialized Text2SQL LLM.
    """
    # Load Config
